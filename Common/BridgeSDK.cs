...
namespace BridgeSDK
{
    ...
    public partial class Controller
    {
        ...
        private delegate bool DrawInteropRGBDTextureDXDelegate(Window wnd, IntPtr dx_texture, uint width, uint height, uint quiltWidth, uint quiltHeight, uint vx, uint vy, float focus, float offset, float aspect, float zoom, int depth_loc);

        public static bool DrawInteropRGBDTextureDX(Window wnd, IntPtr texture, uint width, uint height, uint quiltWidth, uint quiltHeight, uint vx, uint vy, float focus, float offset, float aspect, float zoom, int depth_loc)
        {
            try
            {
                if (GetPlatformID() == PlatformID.Win32NT)
                {
                    var func = DynamicLibraryLoader.LoadFunction<DrawInteropRGBDTextureDXDelegate>(libraryPath, "draw_interop_rgbd_texture_dx");
                    return func(wnd, texture, width, height, quiltWidth, quiltHeight, vx, vy, focus, offset, aspect, zoom, depth_loc);
                }
                return false;
            }
            catch (Exception ex)
